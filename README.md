...
1. `range_location` — position range placement (percent ×100)
2. `distance_to_lower` — price distance to lower tick (percent ×100)
3. `distance_to_upper` — price distance to upper tick (percent ×100)
...
7. `volume_liquidity_ratio_24h` — 24h volume to TVL ratio (percent ×100)
...
| `core-evidence-pipeline` | every 15 min (`*/15 * * * *`) | `pnpm run:core-evidence-pipeline` | collect → derive → assemble → brief → exact publish         |
