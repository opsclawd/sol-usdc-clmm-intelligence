...
import {
  makeMalformedEnvelope,
  makeOnChainFlowSourceSnapshot
...
describe("HttpHeliusFlowSource", () => {
  describe("Helius adapter maps transaction facts without adding motive", () => {
    it("maps transaction facts and drops unknown fields", async () => {
      const mockHttp = createMockHttpClient({
        body: {
          source: "helius-api",
          providerId: "helius-txn-api",
          providerRunId: "helius-run-001",
          ...
          events: [
            {
              ...
              inventedMotive: "should not exist"
            }
          ]
        }
      });
      ...
      const source = new HttpHeliusFlowSource({
        http: mockHttp,
        url: "https://api.helius.com/v0/transactions",
      ...
      expect(result.providerId).toBe("helius-txn-api");
  ...
  describe("bounded-request-with-optional-auth", () => {
    it("appends fromUnixMs and toUnixMs as query parameters", async () => {
      const mockHttp = createMockHttpClient({
        body: makeOnChainFlowSourceSnapshot()
      });
      ...
      expect(mockHttp.getJson).toHaveBeenCalledWith(
        "https://api.helius.com/v0/transactions?fromUnixMs=1699999000000&toUnixMs=1700000000000",
        expect.objectContaining({
          headers: expect.objectContaining({
            Authorization: "Bearer helius-secret-key"
          })
        })
      );
    });
