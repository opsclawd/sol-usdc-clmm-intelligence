...
describe("range calculators", () => {
  ...
  describe("classifies and clamps range location without hiding market state", () => {
    it("returns 0 (percent x100) with below_range_clamped when current is below lower", () => {
      const pos = makePositionState({
        currentPrice: 50,
    ...
    it("returns 10_000 (percent x100) with above_range_clamped when current is above upper", () => {
      const pos = makePositionState({
        currentPrice: 250,
    ...
    it("returns exact midpoint (percent x100) with in_range when current is inside range", () => {
      const pos = makePositionState({
        currentPrice: 150,
        currentPriceLabel: "150",
        lowerPriceLabel: "100",
    ...
    it("returns 0 (percent x100) with at_lower_boundary when current equals lower", () => {
      const pos = makePositionState({
        currentPrice: 100,
    ...
    it("returns 10_000 (percent x100) with at_upper_boundary when current equals upper", () => {
      const pos = makePositionState({
        currentPrice: 200,
  ...
  describe("applies nearest integer ties away from zero after the full formula", () => {
    it("range_location: exact 0.5 position becomes 5000 (percent x100)", () => {
      const pos = makePositionState({
        currentPrice: 150,
        currentPriceLabel: "150",
        lowerPriceLabel: "100",
    ...
    it("distance_to_lower: exact 0.3333... becomes 3333 (percent x100) (50/150 * 10_000)", () => {
      const pos = makePositionState({
        currentPrice: 150,
        currentPriceLabel: "150",
        lowerPriceLabel: "100",
      ...
      expect(result.status).toBe("AVAILABLE");
      const diff = 150 - 100;
      const expectedScaled = (diff / 150) * 10_000;
      expect(result.value).toBe(Math.round(expectedScaled));
    });

    it("distance_to_upper: exact 0.3333... becomes 3333 (percent x100) (50/150 * 10_000)", () => {
      const pos = makePositionState({
        currentPrice: 150,
        currentPriceLabel: "150",
        lowerPriceLabel: "100",
    ...
    it("handles fractional percent x100 value that rounds up", () => {
      const pos = makePositionState({
        currentPrice: 101,
      ...
      expect(result.status).toBe("AVAILABLE");
      const diff = 101 - 100;
      const expectedScaled = (diff / 101) * 10_000;
      const rounded = Math.round(expectedScaled);
      expect(result.value).toBe(rounded);
    });

    it("handles negative fractional percent x100 value that rounds away from zero", () => {
      const pos = makePositionState({
        currentPrice: 99,
      ...
      expect(result.status).toBe("AVAILABLE");
      const diff = 99 - 100;
      const expectedScaled = (diff / 99) * 10_000;
      const rounded = Math.round(expectedScaled);
      expect(result.value).toBe(rounded);
